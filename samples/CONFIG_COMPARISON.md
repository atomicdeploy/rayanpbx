...
### TL;DR: Why We Use Same Names for Extension Sections

RayanPBX uses the **same section name** for endpoint, auth, and aor sections (e.g., `[101]` for all three). This is the **Asterisk-recommended best practice** for SIP user extensions.
...
For SIP trunks:
- Unique names like `[trunk-endpoint]`, `[trunk-auth]` are acceptable
- Trunks use IP matching via `identify` sections, not user registration
- Unique names can provide clarity for complex multi-trunk setups

See: [Asterisk PJSIP Configuration Documentation](https://docs.asterisk.org/Configuration/Channel-Drivers/SIP/Configuring-res_pjsip/PJSIP-Configuration-Sections-and-Relationships/)
...
1. **Auth section naming**: Uses `[100-auth]` with `auth=100-auth`. While this works, it deviates from Asterisk best practices for user extensions. For inbound registration matching, using the same name `[100]` for all sections is recommended.
...
1. **Same-name sections**: Auth section is `[100]` matching Asterisk best practices - sections are distinguished by their `type=` property
