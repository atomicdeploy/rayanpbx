...
EOF
    ...
    # Source the ini-helper if available for proper section manipulation
    local ini_helper=""
    if [ -f "$INSTALL_SCRIPT_DIR/scripts/ini-helper.sh" ]; then
        ini_helper="$INSTALL_SCRIPT_DIR/scripts/ini-helper.sh"
    elif [ -f "/opt/rayanpbx/scripts/ini-helper.sh" ]; then
    ...
    if [ -n "$ini_helper" ] && [ -f "$ini_helper" ]; then
        ...
        # Use INI helper to add or update transport sections
        # First, ensure sections exist
        ensure_ini_section "$pjsip_conf" "transport-udp"
        set_ini_value "$pjsip_conf" "transport-udp" "type" "transport"
        set_ini_value "$pjsip_conf" "transport-udp" "protocol" "udp"
        set_ini_value "$pjsip_conf" "transport-udp" "bind" "0.0.0.0:5060"
        set_ini_value "$pjsip_conf" "transport-udp" "allow_reload" "yes"
        
        ensure_ini_section "$pjsip_conf" "transport-tcp"
        set_ini_value "$pjsip_conf" "transport-tcp" "type" "transport"
        set_ini_value "$pjsip_conf" "transport-tcp" "protocol" "tcp"
        set_ini_value "$pjsip_conf" "transport-tcp" "bind" "0.0.0.0:5060"
        set_ini_value "$pjsip_conf" "transport-tcp" "allow_reload" "yes"
    ...
    # Fallback: Append transport sections directly without managed markers
    # Remove any existing transport-udp or transport-tcp sections first
    # Use awk to remove sections by name (without relying on managed markers)
    local temp_file=$(mktemp)
    awk '
        /^\[transport-udp\]/ { in_section=1; next }
        /^\[transport-tcp\]/ { in_section=1; next }
        /^\[/ { in_section=0 }
        !in_section { print }
    ' "$pjsip_conf" > "$temp_file"
    
    # Append the transport sections
    cat >> "$temp_file" << 'EOF'
...
if next_step "Installation Complete! 🎉" "complete"; then
    ...
    echo ""
    echo -e "  ${GREEN}5.${RESET} ${BOLD}Configure Dialplan${RESET} (required for calls to work):"
    echo -e "     ${DIM}Web UI:${RESET}  Navigate to Dialplan → Create Default Rules"
    echo -e "     ${DIM}TUI:${RESET}     ${WHITE}rayanpbx-tui${RESET} → Dialplan Management → Create Default Pattern"
    echo ""
