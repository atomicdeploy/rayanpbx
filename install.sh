...
if [ ! -f "/etc/asterisk/manager.conf.rayanpbx-configured" ]; then
    ...
    # Capture reload output for diagnostics
    if ! systemctl reload asterisk 2>&1 | tee /tmp/asterisk-reload.log; then
        RELOAD_ERROR="$(cat /tmp/asterisk-reload.log)"
        print_warning "Asterisk reload command reported an issue"
        print_verbose "Reload output captured in /tmp/asterisk-reload.log"
       ...
       sleep 2
        ...
        # Short pause before deeper checks
        sleep 2
        
        # Check status after failed reload
        if systemctl is-active --quiet asterisk; then
            print_info "Asterisk is still running; attempting a controlled restart instead..."
            if systemctl restart asterisk 2>&1; then
                sleep 2
                if check_asterisk_status "Asterisk restart after failed reload" "true"; then
                    print_success "Asterisk restarted successfully after reload failure"
                else
                    print_error "Asterisk restarted but status check failed"
                    handle_asterisk_error "$RELOAD_ERROR" "Asterisk restart post-reload failure"
                fi
            else
                print_error "Failed to restart Asterisk after reload failure"
                print_cmd "systemctl status asterisk  # Inspect current Asterisk state"
                handle_asterisk_error "$RELOAD_ERROR" "Asterisk restart attempt"
            fi
        else
            print_warning "Asterisk does not appear to be running after reload failure"
            print_cmd "asterisk -rvvv  # Launch Asterisk console to investigate"
            handle_asterisk_error "$RELOAD_ERROR" "Asterisk reload (service not active)"
        fi
    else
        print_verbose "Asterisk reload command exited without immediate error"
        sleep 2
        # Double-check health even if reload exited cleanly
        if check_asterisk_status "Asterisk reload after AMI configuration" "true"; then
            print_success "Asterisk configuration reloaded successfully"
        else
            print_warning "Reload completed but status check indicated potential issues"
            
            # Attempt a restart as a remediation path
            print_info "Attempting a full restart to ensure stability..."
            if systemctl restart asterisk > /dev/null 2>&1; then
                sleep 2
                if check_asterisk_status "Asterisk restart after degraded reload state" "true"; then
                    print_success "Asterisk restarted and is healthy"
                else
                    print_error "Asterisk restarted but health check still failing"
                    print_cmd "systemctl status asterisk  # Review service details"
                fi
            else
                print_error "Failed to restart Asterisk after degraded reload state"
                print_cmd "systemctl status asterisk  # Investigate failure cause"
            fi
        fi
    fi
fi
